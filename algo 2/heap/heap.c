...
bool redimensionar_arreglo(heap_t* heap,size_t tam_nuevo){
...
bool heap_encolar(heap_t *heap, void *elem){
	...
	if(heap->cantidad == heap->tamanio){
		if(!redimensionar_arreglo(heap, (heap->tamanio * MULT_TAM))) return NULL;
	}
...
void *heap_desencolar(heap_t *heap){
	...
	if( heap->cantidad <= ( heap->tamanio/ DIV_CAPAC_MIN) ){
		if ( !redimensionar_arreglo(heap,heap->tamanio/ DIV_CAPAC_INTER) ) return NULL;
	}	
	return dato_a_borrar;
}
